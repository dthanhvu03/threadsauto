...
class JobManager:
    ...
    def _check_duplicate_content(
        ...
        for job_id, job in self.jobs.items():
            try:
                # Check account_id và platform match
                if (hasattr(job, 'account_id') and job.account_id == account_id and
                    hasattr(job, 'platform') and job.platform == platform):
    ...
    def add_job(
        ...
        if duplicate_job_id:
            ...
            duplicate_status = getattr(duplicate_job, 'status', None) if duplicate_job else None
            status_str = duplicate_status.value if duplicate_status and hasattr(duplicate_status, 'value') else str(duplicate_status)
    ...
    def list_jobs(
        ...
        try:
            jobs.sort(key=lambda j: (
                j.priority.value if hasattr(j, 'priority') and hasattr(j.priority, 'value') else 0,
            ...
            for j in self.jobs.values():
                try:
                    ...
                    if hasattr(j, 'is_ready') and callable(j.is_ready):
                        ...
                        if self.overdue_threshold_hours is not None and hasattr(j, 'scheduled_time') and j.scheduled_time:
                            from datetime import datetime, timezone
                            from services.utils.datetime_utils import normalize_to_utc
                            scheduled_time_utc = normalize_to_utc(j.scheduled_time)
                            now_utc = datetime.now(timezone.utc)
                            hours_overdue = (now_utc - scheduled_time_utc).total_seconds() / 3600
                        ...
                        # #region agent log - Debug get_ready_jobs
                        import json
                        import os
                        log_path = os.path.join(os.path.expanduser("~"), "threads", ".cursor", "debug.log")
                        with open(log_path, 'a', encoding='utf-8') as f:
                            f.write(json.dumps({"location":"job_manager.py:get_ready_jobs","message":"Job is_ready check result","data":{"job_id":getattr(j,'job_id','unknown')[:8],"status":str(getattr(j,'status','unknown')),"is_ready":is_ready_result,"is_overdue_beyond_threshold":is_overdue_beyond_threshold,"overdue_threshold_hours":self.overdue_threshold_hours},"timestamp":int(__import__('time').time()*1000),"sessionId":"debug-session","runId":"run1","hypothesisId":"F"})+'\n')
                        # #endregion
                        if is_ready_result:
                            ready_jobs.append(j)
                except Exception as e:
    ...
    def cleanup_expired_jobs(
        ...
        try:
            for job_id, job in self.jobs.items():
                try:
                    ...
                    if (hasattr(job, 'is_expired') and callable(job.is_expired) and
                        hasattr(job, 'status')):
                        if (job.is_expired() and 
                            job.status not in [JobStatus.COMPLETED, JobStatus.EXPIRED]):
        ...
        for job_id in expired_jobs:
            try:
                ...
                if (hasattr(job, 'scheduled_time') and 
                    datetime.now() > job.scheduled_time + timedelta(hours=24)):
                    job.status = JobStatus.EXPIRED
                    hours_past = (datetime.now() - job.scheduled_time).total_seconds() / 3600
                    
                    # Format scheduled_time safely
                    scheduled_str = 'N/A'
                    try:
                        if hasattr(job.scheduled_time, 'strftime'):
                            scheduled_str = job.scheduled_time.strftime('%Y-%m-%d %H:%M:%S')
                        else:
                            scheduled_str = str(job.scheduled_time)
                    except Exception:
                        scheduled_str = str(job.scheduled_time) if job.scheduled_time else 'N/A'
                    ...
                    try:
                        self.logger.log_step(
                            ...
                            result="SUCCESS",
                            job_id=job_id,
                            scheduled_time=job.scheduled_time.isoformat() if hasattr(job.scheduled_time, 'isoformat') else str(job.scheduled_time),
