...
class ValidationResult:
    ...
    def has_warnings(self) -> bool:
        ...
        return any(severity == ValidationSeverity.WARNING for severity, _ in self.errors)
    
    def get_error_messages(self) -> List[str]:
        """Lấy danh sách error messages."""
        return [msg for severity, msg in self.errors if severity == ValidationSeverity.ERROR]
    
    def get_warning_messages(self) -> List[str]:
        """Lấy danh sách warning messages."""
        return [msg for severity, msg in self.errors if severity == ValidationSeverity.WARNING]
...
class JobValidator:
    ...
    MAX_SCHEDULE_DAYS_PAST = -1  # Không lên lịch quá khứ (cho phép 1 ngày để handle timezone)
    ...
    def validate_add_job(
        ...
        if not account_id or not isinstance(account_id, str):
            result.add_error(
                ...
                "account_id phải là non-empty string"
            )
        elif len(account_id.strip()) == 0:
            result.add_error(
                ValidationSeverity.ERROR,
                "account_id không được rỗng"
            )
        ...
        if not content or not isinstance(content, str):
            result.add_error(
                ValidationSeverity.ERROR,
                "content phải là non-empty string"
            )
        else:
        ...
        if not isinstance(scheduled_time, datetime):
            result.add_error(
                ValidationSeverity.ERROR,
                "scheduled_time phải là datetime object"
            )
        else:
        ...
        if not isinstance(priority, JobPriority):
            result.add_error(
                ValidationSeverity.ERROR,
                "priority phải là JobPriority enum"
            )
        ...
        if not isinstance(platform, Platform):
            result.add_error(
                ValidationSeverity.ERROR,
                "platform phải là Platform enum"
            )
        ...
        if hasattr(job, 'status'):
            ...
            if status == JobStatus.COMPLETED:
                if not hasattr(job, 'completed_at') or not job.completed_at:
                    result.add_error(
                        ValidationSeverity.WARNING,
                        "COMPLETED job thiếu completed_at"
                    )
            ...
            if status == JobStatus.FAILED:
                if not hasattr(job, 'error') or not job.error:
                    result.add_error(
                        ...
                        "FAILED job không có error message"
                    )
    ...
    def _check_schedule_conflicts(
        ...
        for job in existing_jobs:
            ...
            if job.status in [JobStatus.COMPLETED, JobStatus.EXPIRED, JobStatus.FAILED, JobStatus.CANCELLED]:
                continue
            ...
            if (hasattr(job, 'account_id') and job.account_id == account_id and
                ...
                job_scheduled_utc = normalize_to_utc(job.scheduled_time)
                scheduled_time_utc = normalize_to_utc(scheduled_time)
                time_diff = abs((job_scheduled_utc - scheduled_time_utc).total_seconds())
